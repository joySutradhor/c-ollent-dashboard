...
const Page = () => {
  ...
  const fetchAcademyList = useCallback(async () => {
    ...
    if (!token) return;
    try {
      const res = await axios.get(`${BASE_URL}/academies/`, {
        headers: authHeaders(),
      });
        ...
        <div className="edn__left__right__space">
          ...
          <div className="bg-white shadow-sm rounded-xl p-4 my-16  ">
            <div className="flex items-center justify-between mb-4 ">
              <h2 className="text-lg font-medium">All Academy List</h2>
              ...
              <table className="min-w-full divide-y divide-black/10">
                <thead>
                  <tr className="text-left text-sm text-gray-700">
                    ...
                    <th className="px-3 py-2">Academy Bio</th>
                    <th className="px-3 py-2">Email</th>
                    <th className="px-3 py-2">Number</th>
                    <th className="px-3 py-2">Location</th>
                    <th className="px-3 py-2">Map Link</th>
                  </tr>
                </thead>
                    ...
                    academyList.map((s, index) => (
                      <tr key={s.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-sm">
                            <Image src={s?.academy_thumbnail} alt={s?.academy_name} height={1000} width={1000} className="border border-black/10 rounded size-12 object-cover"/>
                        </td>
                        <td className="px-3 py-2 text-sm">
                          {s?.academy_name}
                        </td>
